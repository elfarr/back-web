...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if (!empty($_COOKIE['save'])) {
    ...
    if (!empty($_COOKIE['pass'])) {
      $messages[] = '<a href="login.php">Войдите</a> с логином <strong>' . 
      $_COOKIE['login'] . '</strong> и паролем <strong>' . $_COOKIE['pass'] . 
  ...
  if ($errors['bio']) {
    ...
    $messages[] = htmlspecialchars('<div class="error">Заполните биографию.</div>');
  }
  if ($errors['tel']) {
    setcookie('tel_error', '', 100000);
    $messages[] = htmlspecialchars('<div class="error">Заполните номер телефона.</div>');
  }
  if ($errors['date']) {
    setcookie('date_error', '', 100000);
    $messages[] = htmlspecialchars('<div class="error">Заполните дату.</div>');
  }
  if ($errors['gen']) {
    setcookie('gen_error', '', 100000);
    $messages[] = htmlspecialchars('<div class="error">Введите пол</div>');
  }
  if ($errors['symbolfio_error']) {
    setcookie('symbolfio_error', '', 100000);
    $messages[] = htmlspecialchars('<div class="error">ФИО содержит недопустимые символы.</div>');
  }

  if ($errors['symboltel_error']) {
    setcookie('symboltel_error', '', 100000);
    $messages[] = htmlspecialchars('<div class="error">Укажите номер телефона в формате +7 (XXX) XXX-XX-XX.</div>');
  }
  if ($errors['languages_error']) {
    setcookie('languages_error', '', 100000);
    $messages[] = htmlspecialchars('<div class="error">Выберите языки.</div>');
  }
  if ($errors['languages_unknown']) {
    setcookie('languages_unknown', '', 100000);
    $messages[] = htmlspecialchars('<div class="error">Ошибка при добавлении языка.</div>');
  }
  if ($errors['date_value_error']) {
    setcookie('date_value_error', '', 100000);
    $messages[] = htmlspecialchars('<div class="error">Заполните дату в формате d.m.Y.</div>');
  }
  if ($errors['bio_value_error']) {
    setcookie('bio_value_error', '', 100000);
    $messages[] = htmlspecialchars('<div class="error">Биография содержит недопустимые символы.</div>');
  }
